...
- `scripts/levene_variance.py` — produces Table 4 (transfer-variance characterization with the classical mean-centered Levene's test, matching the paper).
- `scripts/paired_ttest_holm.py` — produces Table 5 (paired *t*-tests with Holm--Bonferroni correction, family size 24 per metric: 6 model pairs x 4 operating points). Sample sizes follow the multi-seed design: *n* = 3 at zero-shot and fs-20, *n* = 2 at fs-50 and fs-100.
...
python scripts/levene_variance.py       # -> Table 4 (variance characterization)
